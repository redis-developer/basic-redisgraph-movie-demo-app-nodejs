    ...
    return (
        <div className={'card-film'} onClick={handleDetailsOfFilm}>
            ...
            <div className={'film-photo'}>
                {!items.poster? <img src={defaultFilm} alt="defaultIco"/> : <img src={items.poster} alt={items.title}/>}
                <div className={'filmInfo'}>
                    <p>Rating: {items.imdbRating} of 10</p>
                    <div className={'rating'}>
                        <div className={'rating-item'} style={{width: `${items.imdbRating*10}%`}}>

                        </div>
                    </div>
                    <p>{items.runtime} min</p>
                    <p>{items.year}, {items.countries[0]}</p>
                </div>
            </div>
            ...
            <div className={'modalWindow'}>
                {active ? <Modal active={active} setActive={setActive}>
                    <h4>Some problems...</h4>
                    <h4><Link to={'/authorization'}>Sign up</Link> to see the details of the movie</h4>
                </Modal> : null}
            </div>
        </div>
    )
