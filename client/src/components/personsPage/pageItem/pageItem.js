        ...
        <div className={'personInfoPage'}>
            <div className={'personInfoWrapper'}>
                ...
                <div className={'personInfoText'}>
                    ...
                    {info.properties.died? <p>Dead in: {info.properties.died[0]}</p> : null}
                    {info.properties.bornIn ? <p>Born in: {info.properties.bornIn}</p> : null}
                    {info.properties.bio? <p>{info.properties.bio}</p> : <p>Nothing information about this person</p>}
                </div>
            </div>
            <h3>Films of this person: </h3>
            <div className={'film-row-person'}>
                {films.map((value, index) => <MoviesItem key={index} items={value} name={'genre'}/>)}
            </div>
        </div>
    )
};
